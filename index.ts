...
const memoryLanceDBProPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Prefer fully-offline local cross-encoder rerank when requested via env.
    // Defaults for 2C/2G:
    // - If embedding provider is local and no rerankApiKey provided:
    //     * LOCAL_RERANK=1|true  -> use "local-cross-encoder" (requires @xenova/transformers small CE model)
    //     * otherwise             -> use "lightweight" cosine fallback (zero extra model)
    const preferLocalCE =
      (process.env.LOCAL_RERANK || "").toLowerCase() === "1" ||
      (process.env.LOCAL_RERANK || "").toLowerCase() === "true";
    const fallbackRerank = preferLocalCE ? "local-cross-encoder" : "lightweight";
    const envLocalModel = process.env.RERANK_LOCAL_MODEL;
    ...
    const retriever = createRetriever(store, embedder, {
      ...
      !config.retrieval?.rerankApiKey
        ? { rerank: fallbackRerank as any }
        : {}),
      ...(preferLocalCE && envLocalModel ? { rerankModel: envLocalModel } : {}),
    });
    ...
    api.logger.info(
      `memory-lancedb-pro@${pluginVersion}: plugin registered (db: ${resolvedDbPath}, provider: ${config.embedding.provider}, model: ${defaultModel})`,
    );
    ...
    async function runBackup() {
      try {
        ...
        const files = (await readdir(backupDir))
          .filter((f: string) => f.startsWith("memory-backup-") && f.endsWith(".jsonl"))
          .sort();
        if (files.length > 7) {
...
function parsePluginConfig(value: unknown): PluginConfig {
  ...
  // Provider: "openai-compatible" (default) or "local" (no API key required)
  const provider =
    typeof embedding.provider === "string" &&
    (embedding.provider === "local" || embedding.provider === "openai-compatible")
      ? (embedding.provider as "local" | "openai-compatible")
      : "local";

  // Accept single key (string) or array of keys for round-robin rotation (only for openai-compatible)
  ...
  if (provider === "openai-compatible") {
    if (typeof embedding.apiKey === "string") {
      ...
      // No API key provided (and no OPENAI_API_KEY env) — allow automatic fallback to local provider
    }
  }
    ...
    embedding: {
      ...
      model:
        typeof embedding.model === "string"
          ? embedding.model
          : (provider === "local" || (!apiKey && provider === "openai-compatible"))
            ? "all-MiniLM-L6-v2"
            : "text-embedding-3-small",
      baseURL:
