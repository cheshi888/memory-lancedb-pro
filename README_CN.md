...
### 本地微模型（2C/2G VPS，无需外部 API）

无需任何 API Key，即可在纯离线环境工作。使用 @xenova/transformers 在本地 CPU 进行特征提取（mean pooling + 归一化），适合低配 VPS。
...
说明与建议：
- 首次运行会自动下载并缓存模型权重，后续离线可复用。
- 推荐模型：all-MiniLM-L6-v2（384 维）；如需强制维度可设置 `"embedding.dimensions": 384`。
- 2C/2G 环境下已自动将 ONNX/wasm 线程限制为 1–2，降低内存与 CPU 峰值。
- 批量嵌入时建议控制并发，或按默认顺序串行处理批次。
- 若未来切回外部服务，将 `provider` 改回 `"openai-compatible"` 并配置 `apiKey`/`baseURL`。

重排（Rerank）：
- 本地模式建议使用 `"lightweight"`（基于余弦相似度的轻量重排），完全离线。
- 如确有更高质量需求且可访问外网，可提供 `retrieval.rerankApiKey`/`retrieval.rerankEndpoint` 启用交叉编码器重排（cross-encoder）。

自动降级（无 API Key）：
- 未提供 `embedding.apiKey` 且未设置 `OPENAI_API_KEY` 时，即使 `provider` 配置为 `"openai-compatible"`，插件也会自动降级为本地模式（`provider="local"`，模型默认为 `all-MiniLM-L6-v2`）。
- 在该降级模式下，检索的 `rerank` 会自动设置为 `"lightweight"`（若未显式配置 `retrieval.rerankApiKey`）。
- 适用于 2C/2G VPS；后续若添加 API Key，将自动恢复到外部服务。

#### 离线重排（local-cross-encoder）
在完全离线环境下，你也可以启用“小型交叉编码器本地重排”（基于 @xenova/transformers）：
- 开关：设置环境变量 LOCAL_RERANK=1 或 true
- 模型：默认 Xenova/cross-encoder-ms-marco-MiniLM-L-6-v2（也可通过 RERANK_LOCAL_MODEL 指定）
- 资源占用：CPU 1–2 线程、串行打分，适配 2C/2G；失败时自动回退到 "lightweight" 余弦重排
- 首次运行会自动下载并缓存权重，后续离线可复用
...
#（可选）指定本地 CE 模型名
export RERANK_LOCAL_MODEL="Xenova/cross-encoder-ms-marco-MiniLM-L-6-v2"
```
...
```json
{
  ...
  "retrieval": {
    "rerank": "lightweight" // 或者不写，结合 LOCAL_RERANK 开启本地 CE
  }
}
```

提示：
- 首次运行建议在维护时段“预热”一次（会下载嵌入/重排模型），之后完全离线运行
- 也可在未提供任何 API Key 时保持 provider: "openai-compatible"，插件会自动降级为本地模式
