...
### Local micro model (2C/2G VPS, no external API)

Run fully offline without any API keys. Uses @xenova/transformers CPU feature-extraction with mean pooling and normalization. Designed to fit small VPS constraints.
...
Notes:
- First run downloads model weights and caches them on disk for subsequent offline use.
- Recommended model: all-MiniLM-L6-v2 (384 dims). You can force dimensions with `"embedding.dimensions": 384` if desired.
- On 2C/2G machines, ONNX/wasm threads are automatically limited to 1–2 to reduce CPU/memory spikes.
- For batch embedding, keep concurrency low (the local provider in this repo embeds sequentially to save RAM).
- To switch back to external services later, set `provider` to `"openai-compatible"` and provide `apiKey`/`baseURL`.

Rerank:
- In local mode, `"lightweight"` rerank (cosine-based) is recommended and fully offline.
- If you need stronger reranking and have outbound access, provide `retrieval.rerankApiKey`/`retrieval.rerankEndpoint` to enable cross-encoder reranking.
