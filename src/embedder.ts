...
export interface EmbeddingConfig {
  ...
  /** Single API key or array of keys for round-robin rotation with failover (not required for provider: "local"). */
...
export class Embedder {
  ...
  private readonly _provider: "openai-compatible" | "local";
  private localExtractorPromise?: Promise<any>;
  ...
  constructor(config: EmbeddingConfig & { chunking?: boolean }) {
    this._provider = (config.provider ?? "openai-compatible") as "openai-compatible" | "local";
    ...
    if (this._provider === "openai-compatible") {
      // Normalize apiKey to array and resolve environment variables
      const apiKeys = Array.isArray(config.apiKey) ? config.apiKey : [config.apiKey as string];
      ...
      // Local provider: no HTTP clients needed
      this.clients = [];
    }
  ...
  // Local provider helpers (@xenova/transformers)
  private async ensureLocalExtractor(): Promise<(input: string | string[], opts?: Record<string, unknown>) => Promise<any>> {
    if (!this.localExtractorPromise) {
      this.localExtractorPromise = (async () => {
        const { pipeline, env } = await import("@xenova/transformers");
        // Keep resource usage low for 2C/2G machines
        try {
          // Best-effort: limit threads where supported
          // @ts-ignore
  ...
  private async embedLocal(text: string, _task?: string): Promise<number[]> {
    const extractor = await this.ensureLocalExtractor();
    const output = await extractor(text, {
      pooling: "mean",
      normalize: this._normalized ?? true,
    });
    // transformers.js returns { data: Float32Array }
    const raw = Array.from((output?.data || []) as Float32Array);
    // Adjust to requested dimensions if needed (pad or truncate)
    let vec = raw;
    if (this.dimensions && raw.length !== this.dimensions) {
      if (raw.length > this.dimensions) {
        vec = raw.slice(0, this.dimensions);
      } else if (raw.length < this.dimensions) {
        vec = raw.concat(new Array(this.dimensions - raw.length).fill(0));
      }
    }
    return vec;
  }
  ...
  private async embedSingle(text: string, task?: string): Promise<number[]> {
    ...
    if (this._provider === "local") {
      const embedding = await this.embedLocal(text, task);
  ...
  private async embedMany(texts: string[], task?: string): Promise<number[][]> {
    ...
    if (this._provider === "local") {
      ...
      // Compute sequentially to reduce memory spikes on small VPS
      for (let i = 0; i < validTexts.length; i++) {
        const originalIndex = validIndices[i];
        const emb = await this.embedLocal(validTexts[i], task);
