...
export interface RetrievalConfig {
  ...
  bm25Weight: number;
  minScore: number;
  rerank: "cross-encoder" | "lightweight" | "none" | "local-cross-encoder";
...
/**
 * Local cross-encoder reranker (optional, fully offline).
 * Uses @xenova/transformers text-classification pipeline with a small CE model.
 * Default model: Xenova/cross-encoder-ms-marco-MiniLM-L-6-v2 (lightweight).
 * Notes:
 *  - Runs sequentially to limit memory/CPU on 2C/2G machines.
 *  - If unavailable or any error occurs, reranking falls back to cosine.
 */
let localRerankerPromise: Promise<any> | null = null;
async function getLocalReranker(model?: string) {
  if (!localRerankerPromise) {
    localRerankerPromise = (async () => {
      const { pipeline, env } = await import("@xenova/transformers");
      // Keep resource usage low for 2C/2G machines
      try {
        // @ts-ignore
        ...
        // @ts-ignore
        {
          ...
          try {
            ...
            cpuCount = typeof os.cpus === "function" ? os.cpus().length : 1;
          } catch {}
          // Use at most 2 threads
          env.backends.onnx.wasm.numThreads = Math.max(1, Math.min(2, cpuCount));
        }
      } catch {}
      const modelName = model || "Xenova/cross-encoder-ms-marco-MiniLM-L-6-v2";
      return await pipeline("text-classification", modelName);
    })();
  }
  return await localRerankerPromise;
}

async function localCrossEncodeScore(reranker: any, query: string, doc: string): Promise<number> {
  // Primary: pair input
  try {
    const out = await reranker({ text: [query, doc] });
    const score = Array.isArray(out) ? (out[0]?.score ?? 0) : (out?.score ?? 0);
    return Number.isFinite(score) ? Number(score) : 0;
  } catch {
    // Fallback: manually joined input
    try {
      const out = await reranker(`${query} [SEP] ${doc}`);
      const score = Array.isArray(out) ? (out[0]?.score ?? 0) : (out?.score ?? 0);
      return Number.isFinite(score) ? Number(score) : 0;
    } catch {
      return 0;
...
export class MemoryRetriever {
  ...
  private async rerankResults(
    ...
    // Local cross-encoder rerank (fully offline)
    if (this.config.rerank === "local-cross-encoder") {
      try {
        const reranker = await getLocalReranker(this.config.rerankModel);
        const reranked = [];
        for (let i = 0; i < results.length; i++) {
          const r = results[i];
          const s = await localCrossEncodeScore(reranker, query, r.entry.text);
          // Blend: 60% local CE score + 40% original fused score
          const blendedScore = clamp01(s * 0.6 + r.score * 0.4, r.score * 0.5);
          reranked.push({
            ...r,
            score: blendedScore,
            sources: { ...r.sources, reranked: { score: s } },
          });
        }
        return reranked.sort((a, b) => b.score - a.score);
      } catch (e) {
        console.warn("Local cross-encoder rerank failed, falling back to cosine:", e);
        // Continue to cosine fallback below
      }
    }
